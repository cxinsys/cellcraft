...
# Switch to CPU branch with user confirmation
switch_to_cpu_branch() {
    ...
    echo ""
    log_info "Recommended branch for CPU mode: ${CYAN}${CPU_BRANCH}${NC}"
    echo ""

    # Check if already on recommended branch
    if [[ "$current_type" == "branch" && "$current_value" == "$CPU_BRANCH" ]]; then
        log_success "Already on recommended branch: ${CPU_BRANCH}"
        log_info "Proceeding with current branch..."
        cd "${SCRIPT_DIR}"
        return 0
    fi

    # Check if CPU branch exists in remote
    ...
    echo -e "  Recommended: ${GREEN}${CPU_BRANCH}${NC}"
    echo ""
    ...
    case "$user_choice" in
        ...
        *)
            ...
            log_warning "Note: Some CPU-only plugins may not work correctly with this branch"
            echo ""
